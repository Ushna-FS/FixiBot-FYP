...
class MechanicDetailScreen extends StatelessWidget {
  ...
  // ✅ UPDATED: Service confirmation dialog with issue description input
  void _showServiceConfirmationDialog() {
    // This dialog will appear when the user returns to the app after the call
    Future.delayed(Duration(milliseconds: 500), () {
      if (!Get.isDialogOpen!) {
        showDialog(
          ...
          barrierDismissible: false,
          builder: (BuildContext context) {
            return _IssueDescriptionDialog(
              mechanicName: mechanic['full_name'] ?? 'this mechanic',
              onConfirm: (issueDescription) {
  ...
  // ✅ UPDATED: Accept issue description parameter
...
// ✅ NEW: Custom Dialog for Issue Description
class _IssueDescriptionDialog extends StatefulWidget {
  final String mechanicName;
  final Function(String) onConfirm;

  const _IssueDescriptionDialog({
    required this.mechanicName,
    required this.onConfirm,
  });

  @override
  __IssueDescriptionDialogState createState() => __IssueDescriptionDialogState();
}

class __IssueDescriptionDialogState extends State<_IssueDescriptionDialog> {
  ...
  void initState() {
    super.initState();
    _issueController.addListener(_validateInput);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final wordsRemaining = 10 - wordCount;
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text(
              "Did you choose ${widget.mechanicName} for your vehicle service?",
              style: AppFonts.montserratText2,
            ),
            SizedBox(height: 16),
            Text(
              "Please provide detailed breakdown of the issue:",
              style: AppFonts.montserratText2.copyWith(
                fontWeight: FontWeight.w600,
              ),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: "Describe the vehicle issue in detail (minimum 10 words required)...",
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _isButtonEnabled
              ? () {
                  Navigator.of(context).pop();
                  widget.onConfirm(_issueController.text.trim());
                }
              : null,
          style: ElevatedButton.styleFrom(
