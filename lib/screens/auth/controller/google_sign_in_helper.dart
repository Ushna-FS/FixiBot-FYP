...
class GoogleSignInController extends GetxController {
  ...
  Future<bool> _sendTokenToBackendEnhanced(String googleIdToken, GoogleSignInAccount googleUser) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // ✅ FIX: Store both access token and token type
  ...
  Future<void> _handleSuccessfulSignUp(GoogleSignInAccount googleUser, {Map<String, dynamic>? userData}) async {
    try {
      ...
      // ✅ For first-time users, navigate to User Journey instead of Home
...
/// Clear any global custom image references when a new user signs in
Future<void> _clearGlobalCustomImageIfNeeded(String currentUserEmail) async {
  try {
    final prefs = await SharedPreferences.getInstance();
    
    // Remove any legacy global custom image storage
    final globalCustomImage = prefs.getString('custom_profile_image');
    if (globalCustomImage != null) {
      await prefs.remove('custom_profile_image');
      print("🧹 Cleared global custom image reference for new user: $currentUserEmail");
    }
    
    final globalTimestamp = prefs.getString('custom_image_timestamp');
    if (globalTimestamp != null) {
      await prefs.remove('custom_image_timestamp');
      print("🧹 Cleared global custom image timestamp for new user: $currentUserEmail");
    }
  } catch (e) {
    print("❌ Error clearing global custom image references: $e");
  }
}


/// Smart profile image detection with persistent custom image support - UPDATED
Future<void> _updateUserController(GoogleSignInAccount googleUser, Map<String, dynamic>? userData) async {
  try {
    // Clear any previous global custom image references when new user signs in
    await _clearGlobalCustomImageIfNeeded(googleUser.email);
    ...
    if (userData != null) {
      ...
      print("🔍 Profile image analysis for $userEmail:");
      print("   - Backend provided: $profileImageUrl");
      
      // ✅ CRITICAL FIX: Check if we have a stored custom image for THIS SPECIFIC USER
      final String? storedCustomImage = await _getStoredCustomImage();
      print("   - Stored custom image for current user: $storedCustomImage");
      
      if (storedCustomImage != null && storedCustomImage.isNotEmpty) {
        // We have a custom image stored for this specific user - use it
        profileImageUrl = storedCustomImage;
        print("✅ Using stored custom profile image for current user");
        ...
        // Store it for future use for this specific user
        ...
        // Backend has Google image - check if THIS USER has custom image history
        print("ℹ️ Backend returned Google image, checking for custom image preference for current user...");
        
        // If THIS USER previously had a custom image, we should preserve that preference
        final bool hasCustomImageHistory = await _hasCustomImageHistory();
        if (hasCustomImageHistory) {
          // Try to get the custom image for THIS USER
          ...
          if (previousCustomImage != null && previousCustomImage.isNotEmpty) {
            ...
            print("✅ Restoring previous custom image for current user: $profileImageUrl");
          }
        } else {
          // Use Google image for this user
          profileImageUrl = googleUser.photoUrl;
          print("ℹ️ Using Google profile image for current user (no custom image history)");
        }
      } else if (profileImageUrl == null || profileImageUrl.isEmpty) {
        // Fallback to Google image
        profileImageUrl = googleUser.photoUrl;
        print("ℹ️ Using Google profile image as fallback for current user");
      }
    } else {
      userName = googleUser.displayName ?? 'Google User';
      // Check for stored custom image for THIS USER first
      final String? storedCustomImage = await _getStoredCustomImage();
      profileImageUrl = storedCustomImage ?? googleUser.photoUrl;
      print(storedCustomImage != null 
          ? "✅ Using stored custom image for current user (no backend data)" 
          : "ℹ️ Using Google profile image for current user (no backend data)");
    }
    
    print('👤 Final UserController update for $userEmail:');
    print('   Name: $userName');
    ...
    // Ensure the custom image is properly stored for THIS USER for future logins
...
Future<String?> _getStoredCustomImage() async {
  try {
    final prefs = await SharedPreferences.getInstance();
    final userEmail = prefs.getString('email'); // Get current user's email
    
    if (userEmail != null && userEmail.isNotEmpty) {
      final userSpecificKey = 'custom_profile_image_${userEmail.toLowerCase()}';
      final customImage = prefs.getString(userSpecificKey);
      print("📥 Loaded stored custom image for $userEmail: ${customImage ?? 'None'}");
      return customImage;
    } else {
      print("❌ Cannot get custom image: No user email found");
      return null;
    }
  } catch (e) {
...
Future<void> storeCustomProfileImage(String imageUrl) async {
  try {
    ...
    if (userEmail != null && userEmail.isNotEmpty) {
      // Store custom image with user email as key
      final userSpecificKey = 'custom_profile_image_${userEmail.toLowerCase()}';
      await prefs.setString(userSpecificKey, imageUrl);
      
      // Also store a timestamp with user email
      final timestampKey = 'custom_image_timestamp_${userEmail.toLowerCase()}';
      await prefs.setString(timestampKey, DateTime.now().toIso8601String());
      
      print("✅ Custom profile image stored for user $userEmail: $imageUrl");
      
      // Verify the save
      final verify = prefs.getString(userSpecificKey);
      print("🔍 Custom image save verification for $userEmail: $verify");
    } else {
      print("❌ Cannot store custom image: No user email found");
    }
  } catch (e) {
...
/// Get previous custom image from various possible sources - UPDATED with user-specific
Future<String?> _getPreviousCustomImage() async {
  try {
    ...
    // Try user-specific stored custom image first
    final userSpecificKey = 'custom_profile_image_${userEmail.toLowerCase()}';
    final storedCustom = prefs.getString(userSpecificKey);
    if (storedCustom != null && storedCustom.isNotEmpty) {
      return storedCustom;
    }
    
    // Try profile_image_url from shared prefs (legacy support)
...
/// Sign-Out Method - UPDATED to clear user-specific custom images
Future<void> signOut() async {
  try {
    isLoading.value = true;

    // Get current user email before clearing data
    final prefs = await SharedPreferences.getInstance();
    final userEmail = prefs.getString('email');
    ...
    // Clear user-specific custom image data
    if (userEmail != null && userEmail.isNotEmpty) {
      final userSpecificKey = 'custom_profile_image_${userEmail.toLowerCase()}';
      final timestampKey = 'custom_image_timestamp_${userEmail.toLowerCase()}';
      await prefs.remove(userSpecificKey);
      await prefs.remove(timestampKey);
      print("🧹 Cleared custom image data for signed out user: $userEmail");
    }
